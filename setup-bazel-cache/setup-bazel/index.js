...
async function setupBazel() {
  ...
  if (config.skipDiskCacheRestore) {
    core.info('Skipping disk-cache restore (skip-disk-cache-restore: true)')
  } else {
    await restoreCache(config.diskCache)
  }
  if (config.skipRepositoryCacheRestore) {
    core.info('Skipping repository-cache restore (skip-repository-cache-restore: true)')
  } else {
