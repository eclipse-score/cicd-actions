...
test('malformed profiles are rejected instead of producing a partial summary', () => {
  assert.throws(
    () => summarizeProfile({ traceEvents: 'not-an-array' }),
  ...
  assert.throws(
    () => summarizeProfile({ traceEvents: [{ cat: 'build phase marker', name: 'Initialize command' }] }),
