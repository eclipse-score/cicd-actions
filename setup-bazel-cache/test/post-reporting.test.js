...
function runPost(
  ...
  const output = execFileSync(process.execPath, [
    ...
    env: {
      ...
      'INPUT_REPORT-CACHE-STEP-SUMMARY': 'true',
...
test('disabling the step summary keeps the job log but leaves the step summary empty', (context) => {
  const root = fixture(context);
  const { output, summary } = runPost(
    root,
    true,
    undefined,
    undefined,
    { 'INPUT_REPORT-CACHE-STEP-SUMMARY': 'false' },
  );
  assert.match(output, /Cache saving is disabled on this ref/);
  assert.doesNotMatch(output, /::warning::|::error::/);
...
test('summary groups repeated invocations with their target patterns', (context) => {
  ...
  // The job log must stand on its own since the step summary is optional
  // (report-cache-step-summary); it needs the same targets and duration.
  assert.match(output, /Bazel 000-test cache: 1 \/ 1 cached \(100%\) · 12\.4 s/);
  assert.match(output, /Targets: \/\/:test-0/);
  // Same reasoning for the test-cache table, which has no target column.
  assert.match(output, /Per-invocation targets:\n\s+000-test: \/\/:test-0/);
});
