...
import { logLocalCacheSize } from './cache-size.js';
...
import { writeCacheStepSummaryEnabled } from './report.js';
import {
...
async function run() {
  try {
    ...
    // Validated here for fail-fast parity with the other reporting inputs;
    // the post step reads this input again directly when writing the summary.
    writeCacheStepSummaryEnabled(core.getInput('report-cache-step-summary'));
    const rawCacheSaveBranchPatterns = core.getInput('cache-save-branch-patterns');
    // A failure here (for example an unresolvable repository default branch)
    // must only disable cache saving, not the cache restore below, which does
    // not depend on the save-branch policy at all.
    let cacheSaveBranchPatterns;
    try {
      cacheSaveBranchPatterns = parseCacheSaveBranchPatterns(
        rawCacheSaveBranchPatterns,
        rawCacheSaveBranchPatterns.trim() ? undefined : resolveDefaultBranch(),
      );
    } catch (error) {
      core.warning(`Cache saving disabled: ${error.stack || error}`);
