...
/**
 * Locate the first Bazel command after startup options. Startup options may
 * take their value as a separate token (`--output_base /tmp/x`) rather than
 * `--output_base=/tmp/x`; since Bazel's set of value-taking startup options
 * is not tracked here, a non-option token that is not a measured command is
 * treated as such a value and skipped, rather than ending the search.
 */
